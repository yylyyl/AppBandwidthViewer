...
void ui_print() {
    ...
    if(options.showhelp) {
	  ...
	  FILE *my_file = fopen("/tmp/iftop.txt","w");
	  if(i == 0 || nn != NULL)
	  {
	  	while( (nn = sorted_list_next_item(&screen_list, nn)) != NULL )
	  	{
	  		host_pair_line* screen_line = (host_pair_line*)nn->data;
	  		sprint_host(host1, &(screen_line->ap.src), screen_line->ap.src_port, screen_line->ap.protocol, 22);
            sprint_host(host2, &(screen_line->ap.dst), screen_line->ap.dst_port, screen_line->ap.protocol, 22);
	  		float out = screen_line->sent[options.bandwidth_in_bytes]/1024;
	    	float in  = screen_line->recv[options.bandwidth_in_bytes]/1024;
	  		fprintf(my_file, "%s %s %.1f %.1f\n", host1, host2, out, in);
	  	}
	  }
	  fclose(my_file);
